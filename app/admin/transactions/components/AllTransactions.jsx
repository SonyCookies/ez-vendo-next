...
export default function AllTransactions() {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        if (filterType === "all" || filterType === "manual") {
          try {
            ...
            querySnapshot.forEach((doc) => {
              ...
              const transactionId = formatTransactionIdHelper(doc.id);
        ...
        // Fetch top-up requests (approved/rejected)
        if (filterType === "all" || filterType === "topup_request") {
          try {
  ...
  // Format transaction ID helper
  const formatTransactionIdHelper = (documentId) => {
  ...
  const formatDateTimeHelper = (timestamp) => {
    if (!timestamp) return "";
          ...
          <div className="flex flex-col sm:flex-row gap-4 p-4 sm:p-5 rounded-2xl bg-gray-50 border border-dashed border-gray-300">
            ...
            <form className="flex flex-col gap-1 flex-1">
              ...
              >
                ...
                <option value="topup_request">Top-up Requests</option>
              </select>
            </form>
