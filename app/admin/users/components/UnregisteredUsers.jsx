...
export default function UnregisteredUsers() {
  ...
  const [lastAutoOpenedQuery, setLastAutoOpenedQuery] = useState(""); // Track last query that triggered auto-open
  ...
  const handleClear = () => {
    setSearchQuery("");
    setDebouncedQuery("");
    setLastAutoOpenedQuery(""); // Reset the auto-opened query tracker
    setCurrentPage(1);
  };
  ...
  // Auto-open modal if exactly one user matches the search
  useEffect(() => {
    // Only auto-open if:
    // 1. There's a search query
    // 2. There's exactly one filtered result
    // 3. No modal is currently open
    // 4. This is a new search query (not the same one that was already auto-opened)
    if (effectiveQuery && filteredItems.length === 1 && !selectedUser && effectiveQuery !== lastAutoOpenedQuery) {
      // Small delay to ensure the search has settled
      const timer = setTimeout(() => {
        openUserModal(filteredItems[0]);
        setLastAutoOpenedQuery(effectiveQuery); // Mark this query as auto-opened
      }, 300);
      
      return () => clearTimeout(timer);
    }
  }, [effectiveQuery, filteredItems, selectedUser, lastAutoOpenedQuery]);
  ...
  const anyModalOpen = selectedUser !== null || showResetConfirm || nfcModal;
  useEffect(() => {
